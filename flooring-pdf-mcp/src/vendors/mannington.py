...
class ManningtonVendor(BaseVendor):
    vendor_id = "mannington"
    vendor_name = "Mannington"
    base_url = "https://www.mannington.com"
    aliases = [
        "mannington mills", "mannington carpet",
        "mannington hardwood", "mannington vinyl",
    ]
    ...
    async def search_product(
        ...
        clean_query = self.strip_vendor_prefix(query, "mannington")

        search_url = f"{self.base_url}/search?q={clean_query.replace(' ', '+')}"
        logger.info(f"Mannington search: {search_url}")
        ...
        try:
            ...
            await page.wait_for_selector(
                "[class*='product'], [class*='ProductCard'], .product-tile",
                timeout=15000,
            )
            ...
            if not candidates:
                logger.warning("Mannington: no product cards found")
                return None
            ...
            logger.error(f"Mannington search failed: {exc}")
        return None
    ...
    async def get_pdf_links(
        ...
        try:
            ...
            for tab_text in ["Documents", "Resources", "Downloads", "Specs", "Literature"]:
                try:
            ...
            logger.error(f"Mannington get_pdf_links failed: {exc}")
            return {}
